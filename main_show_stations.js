...
function getShapeSegmentBetweenNodes(nodeA, nodeB) {
  ...
  Object.entries(window.metroShapes).forEach(([shapeId, coords]) => {
    ...
    if (score < bestScore) {
      ...
      if (indexA < indexB) {
        bestSegment = coords.slice(indexA, indexB + 1);
      } else {
        bestSegment = coords.slice(indexB, indexA + 1).reverse();
      }
    }
  });
...
function buildPathLatLngsByShapes(pathNodeIds) {
  ...
  for (let i = 0; i < pathNodeIds.length - 1; i++) {
    ...
    const segment = getShapeSegmentBetweenNodes(nodeA, nodeB);

    if (segment && segment.length >= 2) {
      if (finalCoords.length > 0) {
        finalCoords.push(...segment.slice(1));
      } else {
        finalCoords.push(...segment);
      }
    } else {
      // Nếu không tìm được shape thì fallback nối thẳng 2 ga
      if (finalCoords.length === 0) {
        finalCoords.push([nodeA.lat, nodeA.lon]);
      }
      finalCoords.push([nodeB.lat, nodeB.lon]);
    }
  }
...
function drawPath(pathNodeIds, costWithFactors, realDistance, algorithmUsed) {
  ...
  const latlngs = buildPathLatLngsByShapes(pathNodeIds).filter(
    (p) => p !== null && typeof p[0] === "number" && typeof p[1] === "number",
  );
  if (latlngs.length < 2) {
    console.warn(
  ...
  guestPathPolyline = L.polyline([latlngs[0]], {
  ...
  function animatePathDrawing() {
    if (
      currentIndex < latlngs.length &&
      guestPathPolyline &&
      map.hasLayer(guestPathPolyline)
    ) {
      guestPathPolyline.addLatLng(L.latLng(latlngs[currentIndex]));
      segmentsDrawn++;

      if (segmentsDrawn % 20 === 0 || segmentsDrawn === totalSegments) {
        // Cập nhật view sau mỗi 5 đoạn hoặc khi kết thúc
        map.panTo(L.latLng(latlngs[currentIndex]), {
          animate: true,
          duration: 0.3,
        });
      }
