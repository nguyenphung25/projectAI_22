...
NODES_JS_PATH = os.path.join(BASE_DIR, "data", "fileJs", "nodes_latlon.js")
...
def _load_nodes(path: str) -> Dict[str, Dict[str, float]]:
    text = open(path, "r", encoding="utf-8").read()
    m = re.search(r"window\.nodesLatLon\s*=\s*(\[.*\])\s*;?\s*$", text, re.S)
    if not m:
        raise ValueError(f"Cannot parse {path}")
    raw_text = m.group(1)
    # JS uses unquoted keys; convert id:, name:, lat:, lon: to quoted form
    raw_text = re.sub(r"(\b)(id|name|lat|lon)(\s*):", r'\1"\2"\3:', raw_text)
...
def _waypoints(path: List[str]) -> List[dict]:
    return [
        {"id": n, "name": NODES[n]["name"], "lat": NODES[n]["lat"], "lon": NODES[n]["lon"]}
        for n in path
        if n in NODES
    ]
...
def stations():
    return jsonify(sorted(
        [{"id": nid, "name": n["name"], "lat": n["lat"], "lon": n["lon"]}
         for nid, n in NODES.items()],
        key=lambda x: x["name"],
    ))
...
def state():
    return jsonify({
        "stations": [{"id": nid, "name": n["name"], "lat": n["lat"], "lon": n["lon"]}
                     for nid, n in NODES.items()],
