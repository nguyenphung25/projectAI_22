...
function pointInPolygonGeoJSON(lat, lng, polygonCoords) {
  ...
  for (let i = 1; i < polygonCoords.length; i++) {
    if (pointInRing(lat, lng, polygonCoords[i])) return false;
  }

  return true;
}

function isInsideDCBoundary(lat, lng) {
  if (!dcBoundary) return true;

  const features = dcBoundary.features || [];
  ...
  for (const feature of features) {
    ...
    if (geometry.type === "Polygon") {
      if (pointInPolygonGeoJSON(lat, lng, geometry.coordinates)) {
        return true;
      }
    }
    ...
    if (geometry.type === "MultiPolygon") {
      for (const polygon of geometry.coordinates) {
        if (pointInPolygonGeoJSON(lat, lng, polygon)) {
          return true;
        }
      }
    }
...
/* Use real rail geometry from data/fileJs/metro_shapes.js */
function distSq(a, b) {
  return (a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2;
}

function nearestShapeIndex(shape, point) {
  let bestIndex = 0;
...
function getShapeSegmentBetweenStations(a, b) {
  const shapes = window.metroShapes || {};
  ...
  Object.values(shapes).forEach((shape) => {
    ...
    const i = nearestShapeIndex(shape, start);
    const j = nearestShapeIndex(shape, end);
    const score = distSq(shape[i], start) + distSq(shape[j], end);

    if (score < bestScore) {
      bestScore = score;
      bestSegment =
        i <= j ? shape.slice(i, j + 1) : shape.slice(j, i + 1).reverse();
    }
  });

  return bestSegment && bestSegment.length >= 2 ? bestSegment : [start, end];
}

function buildRealPathGeometry(pathIds) {
  const coords = [];
  for (let i = 0; i < pathIds.length - 1; i++) {
    const a = stationById[pathIds[i]];
    const b = stationById[pathIds[i + 1]];
    if (!a || !b) continue;

    const segment = getShapeSegmentBetweenStations(a, b);
    if (coords.length && segment.length) coords.push(...segment.slice(1));
    else coords.push(...segment);
  }
  return coords;
}
...
function showToast(message, kind = "error", duration = 2000) {
  const el = $("toast");
  if (!el) return;
  ...
  stations.forEach((s) => {
    L.marker([s.lat, s.lon], {
      icon: L.divIcon({
        ...
        html: '<div style="width:10px;height:10px;background:#3b82f6;border-radius:20%;box-shadow:0 1px 4px rgba(0,0,0,0.6);border:1px solid rgba(255,255,255,0.3)"></div>',
        iconSize: [10, 10],
        iconAnchor: [5, 5],
        popupAnchor: [0, -5],
      }),
...
function drawRailway() {
  ...
  const shapes = window.metroShapes || {};
  const shapeList = Object.values(shapes).filter(
    (shape) => Array.isArray(shape) && shape.length >= 2,
  );
  ...
  // fallback if metro_shapes.js is missing
  railwayEdges.forEach((seg) =>
    L.polyline(seg, { color: "#2a3550", weight: 2, opacity: 0.9 }).addTo(
      railwayLayer,
    ),
  );
}
...
function getSetupRealCoords(s) {
  if (s.start_id && s.end_id) {
    const realCoords = buildRealPathGeometry([s.start_id, s.end_id]);
...
function redrawSetups() {
  ...
  setups.forEach((s) => {
    if (s.type === "road") {
      getSetupRealCoords(s).forEach((seg) =>
        L.polyline(seg, {
          color: "#ff2d2d",
          weight: 5,
          opacity: 0.95,
...
document.addEventListener("click", (e) => {
  if (
    !$("stationSearch").contains(e.target) &&
...
function placeNext(lat, lng, name) {
  // Chỉ chặn ngoài boundary khi đang tìm đường
  // Admin / SetupMap vẫn được chọn ngoài boundary
  if (currentMode === "FindPath" && !isInsideDCBoundary(lat, lng)) {
    setStatus("⚠ Start/end point must be inside Washington DC boundary.");
    showToast(
      "Please select start/end point inside Washington DC boundary.",
      "warn",
      2000,
    );
    return;
  ...
  if (clickCoords.length === 1) {
    ...
    startMarker = L.marker([lat, lng], { icon: pinIcon(color, label) })
      .addTo(resultLayer)
      .bindPopup(
        `<b>${setup ? "Station A" : "Start"}${name ? ": " + name : ""}</b>`,
      )
      .openPopup();
    setStatus(
      setup
        ? "Station A picked — click station B"
...
/* ───── Mode + admin toggle ───── */
function syncModeRadioUI() {
  const findRadio = document.querySelector(
    'input[name="mode"][value="FindPath"]',
  );
  const setupRadio = document.querySelector(
    'input[name="mode"][value="SetupMap"]',
  );

  if (currentMode === "SetupMap") {
    if (setupRadio) setupRadio.checked = true;
  } else {
    if (findRadio) findRadio.checked = true;
  }
...
function refreshMode() {
  if (!isAdmin && currentMode === "SetupMap") {
    currentMode = "FindPath";
  }

  const setup = currentMode === "SetupMap";

  const findPathSection = $("findpath-section");
  if (findPathSection) findPathSection.style.display = setup ? "none" : "block";

  const setupSection = $("setup-section");
  if (setupSection) setupSection.style.display = setup ? "block" : "none";

  // Guest: ẩn phần lựa chọn Find Path / Setup. Admin: hiện lại.
  const modeChoices = document.querySelector(".mode-radio-group");
  if (modeChoices) {
    modeChoices.style.display = isAdmin ? "flex" : "none";
  }

  const modeBadge = $("mode-badge");
  if (modeBadge) {
    modeBadge.className = setup ? "setup" : "find";
    modeBadge.textContent = setup ? "SETUP MODE" : "FIND PATH";
  }

  const hintText = $("hint-text");
  if (hintText) {
    hintText.textContent = setup
...
if (roleToggle) {
  roleToggle.addEventListener("change", function () {
    ...
    if (roleLabel) roleLabel.textContent = isAdmin ? "ADMIN" : "GUEST";

    // Khi về Guest, luôn ép về Find Path và ẩn Setup.
    if (!isAdmin) {
      currentMode = "FindPath";
    }
...
// Trạng thái mặc định: Guest, không hiện lựa chọn Find Path / Setup.
isAdmin = roleToggle ? roleToggle.checked : false;
currentMode = isAdmin ? currentMode : "FindPath";
const roleLabel = $("roleLabel");
if (roleLabel) roleLabel.textContent = isAdmin ? "ADMIN" : "GUEST";
refreshMode();
...
function runPathfind() {
  ...
  fetch(`${API}/find_path`, {
    ...
    .then((res) => {
      ...
      const pathCoords = buildRealPathGeometry(res.path || []);
      ...
      // 4) Small dots only on intermediate stations (not every shape point)
      ...
      (res.waypoints || []).forEach((w) => {
        if (w.transfer) {
          ...
          t.className = "wp-transfer";
          t.innerHTML = `↳ Chuyển tuyến: <b>${w.from_route}</b> → <b>${w.to_route}</b>`;
          wp.appendChild(t);
        }
        ...
        const routeText = w.route_short_name
          ? `<span class="wp-route">${w.route_short_name}</span>`
          : "";
      ...
      $("waypoints-section").style.display =
        res.waypoints && res.waypoints.length ? "block" : "none";

      setStatus(
        `✅ ${res.cost_km} km / ~${res.travel_min} min via ${res.algorithm}`,
      );
    })
    .catch((e) => {
      const msg =
...
$("btnCompare").addEventListener("click", () => {
  ...
  fetch(`${API}/compare_path`, {
    ...
    .then((results) => {
      ...
      const drawable = results.filter(
        (r) => !r.error && r.path && r.path.length >= 2,
      );
      drawable
        .slice()
        .sort(
          (a, b) =>
            algoStyle(b.algorithm).weight - algoStyle(a.algorithm).weight,
        )
        .forEach((r) => {
          const s = algoStyle(r.algorithm);
          const realCoords = buildRealPathGeometry(r.path || []);
          if (realCoords.length < 2) return;
      ...
      if (drawable.length) {
        const allCoords = drawable.flatMap((r) =>
          buildRealPathGeometry(r.path || []),
        );
      ...
      $("compare-table").innerHTML =
        ...
        results
          .map((r) => {
            ...
            const cls =
              best && r.algorithm === best.algorithm ? ' class="best"' : "";
      ...
      setStatus(
        `📊 Comparison done — ${drawable.length}/${results.length} paths drawn.`,
      );
    })
    .catch((e) => {
      const msg =
...
function setSetupMode(mode) {
  ...
  if (mode === "road")
    setStatus("Road Closure armed: click point A, then point B.");
  else if (mode === "area")
    setStatus("Area Closure armed: click points (≥3), then Submit.");
  else if (mode === "congestion")
    setStatus("Congestion armed: click point A, then point B.");
}
...
function loadDCBoundary() {
  return fetch("data/dc_boundary.geojson")
    .then((r) => {
      if (!r.ok) throw new Error("Cannot load dc_boundary.geojson");
      return r.json();
    })
    .then((geojson) => {
      dcBoundary = geojson;

      dcBoundaryLayer = L.geoJSON(dcBoundary, {
        style: {
          color: "#2563eb",
          weight: 2,
          opacity: 0.9,
    ...
    .catch((e) => {
      ...
      setStatus("⚠ Cannot load DC boundary. Boundary restriction disabled.");
    });
}
...
fetch(`${API}/state`)
  ...
  .then((s) => {
    ...
    renderSetupList();
    setStatus(
      `Loaded ${stations.length} stations. Click map twice: start → end.`,
    );
  })
  .catch((e) => {
    const msg =
