...
def dijkstra(
    ...
    all_nodes_in_graph: Set[str] = set(adj_list.keys())
    ...
    distances: Dict[str, float] = {
        node: float("inf") for node in all_nodes_in_graph
    }
    distances[source] = 0.0 
    
    predecessors: Dict[str, Optional[str]] = {
        node: None for node in all_nodes_in_graph
    }
    ...
    while priority_queue:
        ...
        if current_vertex == destination:
            ...
            path: List[str] = []
            temp_node: Optional[str] = destination
        ...
        if current_vertex in adj_list:
            for neighbor, weight in adj_list[current_vertex].items():
                ...
                if distance_through_current < distances[neighbor]:
                    ...
                    heapq.heappush(
                        priority_queue,
                        (distance_through_current, neighbor)
                    )
