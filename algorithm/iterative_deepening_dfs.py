...
def _dls_iterative(
    ...
    destination: str,
    depth_limit: int,
    explored_nodes_accumulator: List[str],
    visited_overall_for_explored_order: Set[str]
) -> Optional[List[str]]:

    stack: List[Tuple[str, List[str], int]] = [(source, [source], 0)]
...
def iddfs(
    ...
    explored_nodes_overall: List[str] = []
    visited_overall_for_explored_order: Set[str] = set()

    for depth in range(max_depth + 1):
        path_found = _dls_iterative(
            adj_list,
            source,
            destination,
            depth,
            explored_nodes_overall,
            visited_overall_for_explored_order
        )
